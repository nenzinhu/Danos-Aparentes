...
// Banner de cookies — some do hero marketing
try {
  const reject = page.getByRole('button', { name: /Recusar|Aceitar/i }).first()
  if (await reject.isVisible({ timeout: 2500 })) await reject.click({ timeout: 2000 })
} catch {
  /* sem banner */
}
