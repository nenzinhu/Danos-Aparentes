...
function messageFor(category: ComparisonItem['category'], prev?: DamageRecord, curr?: DamageRecord): string {
  switch (category) {
    case 'unchanged':
 ...
 *   (ambos os lados que não casaram por identity naquela peça)
 *
...
export function compareInspections(
  ...
  // Deep-ish freeze check: work on copies of damage lists only
  ...
  // Unmatched on same part/location with different types → uncertain pairs
