...
export async function POST(req: NextRequest) {
  try {
    ...
    const allowNoDamage = Boolean(body.allowNoDamage);
...
Se NÃO houver dano aparente claro nesta face, responda exatamente:
{"tipo_dano": null, "severidade": null, "descricao": "", "sem_dano": true}`
      : `
...
"descricao": até 40 palavras, em Português, técnica e objetiva (tipo específico + extensão relativa à peça + profundidade/exposição aparente). NUNCA invente uma medida em cm/mm — descreva só proporção relativa à peça. NUNCA mencione preço, valor, custo ou qualquer cifra monetária.${noDamageRule}`;
    ...
    if (
      allowNoDamage &&
      (parsedResult.sem_dano === true ||
        parsedResult.tipo_dano === null ||
        parsedResult.tipo_dano === 'null' ||
        parsedResult.tipo_dano === '')
    ) {
      return NextResponse.json({
        type: null,
        severity: null,
