...
function DonutMix({
  ...
  const r = 30
  const c = 2 * Math.PI * r
  const { arcs } = segments.reduce<{ arcs: Array<{ label: string; pct: number; color: string; dash: string; off: number }>; offset: number }>(
    (acc, s) => {
      const len = (s.pct / 100) * c
      acc.arcs.push({ ...s, dash: `${len} ${c - len}`, off: -acc.offset })
      acc.offset += len
      return acc
    },
    { arcs: [], offset: 0 },
