...
function VehicleInfoFormComponent({ info, onChange, collapsed, onToggleCollapse, onVehicleTypeDetected, resetToken, onWizardComplete, onPlateConfirmed }: Props) {
  ...
  // Enriquecimento FIPE REAL: quando marca+modelo+ano estão preenchidos e ainda
  // não há cotação, consulta a tabela FIPE e preenche info.fipe (sem travar o form).
  const fipeTimer = useRef<ReturnType<typeof setTimeout> | null>(null)
  const fipeInFlight = useRef(false)
  useEffect(() => {
    const brand = info.brand?.trim()
    const model = info.model?.trim()
    const year = info.ano?.trim()
    const hasFipe = info.fipe && (info.fipe.valor || (info.fipe as unknown as Record<string, unknown>)?.valor)
    if (!brand || !model || !year || hasFipe || fipeInFlight.current) return

    if (fipeTimer.current) clearTimeout(fipeTimer.current)
    fipeTimer.current = setTimeout(async () => {
      fipeInFlight.current = true
      try {
        const params = new URLSearchParams({
          brand,
          model,
          year,
          type: info.vehicleTypeDesc || 'car',
        })
        const res = await fetch(`/api/fipe-lookup?${params.toString()}`, { cache: 'no-store' })
        if (!res.ok) return
        const data = await res.json()
        if (data?.found && data?.valor) {
          onChange({
            ...info,
            fipe: {
              textoMarca: data.marca,
              textoModelo: data.modelo,
        ...
        // fail-silent: FIPE é enriquecimento opcional
      } finally {
        fipeInFlight.current = false
      }
    }, 700)
    return () => {
      if (fipeTimer.current) clearTimeout(fipeTimer.current)
    }
  }, [info.brand, info.model, info.ano, info.vehicleTypeDesc, info.fipe, onChange, info])
