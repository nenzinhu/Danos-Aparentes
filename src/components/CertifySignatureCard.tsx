...
import { generatePdfBlob } from '../lib/pdf/render'
import { captureSvgs, resolvePdfSettings, loadSectionsConfig } from './reportActions/pdfExport'
import type { VehicleInfo, Damage, VehicleType } from '../types'
...
interface Props {
  ...
  /** Dados do laudo para gerar o PDF na certificação (quando disponíveis). */
...
export default function CertifySignatureCard({
  ...
  const handleCertify = useCallback(async () => {
    ...
    try {
      ...
      // Gera o PDF do laudo no client (jspdf) e converte para base64,
      // para a rota de certificação enviá-lo à Assinafy.
      let pdfBase64 = ''
      if (vehicleInfo && damages) {
        try {
          const vType = (vehicleType || vehicleInfo.vehicleTypeDesc || 'car') as VehicleType
          const svgData = await captureSvgs(vType, damages)
          ...
          const blob = await generatePdfBlob(vehicleInfo, damages, svgData, settings)
          ...
          console.warn('[certify] falha ao gerar PDF local:', pdfErr)
        }
      }
