...
const DEFAULT_SECTIONS: SectionVisibilityState = {
  showInfoTable: true,
...
export default function ReportActions({
  ...
  const [sectionsConfig, setSectionsConfig] = useState<SectionVisibilityState>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('pdf_sections_config')
      if (saved) {
        try { return { ...DEFAULT_SECTIONS, ...JSON.parse(saved) } } catch {}
      }
    }
  ...
  const updateSectionsConfig = (newConfig: SectionVisibilityState) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('pdf_sections_config', JSON.stringify(newConfig))
    }
  }

  const getResolvedPdfSettings = () => {
  ...
  async function handle(key: string, fn: () => Promise<boolean | void>, successMsg?: string) {
    ...
    try {
      const ok = await fn()
      // Early-return guards (revisão / cota) return false — don't toast success.
      if (ok === false) return
      if (successMsg && onToast) onToast(successMsg)
    } catch (e) {
      console.error(e)
      const msg = e instanceof Error && e.message ? e.message : 'Erro ao gerar arquivo'
  ...
  async function whatsappPdf(): Promise<boolean> {
    ...
    const resolvedVehicleInfo = { ...vehicleInfo, interiorPhotos: await resolvePhotos(vehicleInfo.interiorPhotos) }
    const pdfSettings = getResolvedPdfSettings()
    const blob = await generatePdfBlob(resolvedVehicleInfo, resolvedDamages, svgData, pdfSettings)
  ...
  return (
    <div className="space-y-3">
        ...
        >
          ...
          <p className="text-[0.72rem] text-[var(--text-muted)] mb-2.5 leading-relaxed">
            {isReviewed
              ...
              : 'Confirme que revisou avarias, fotos, GPS e assinaturas antes de gerar o PDF emitido.'}
          </p>
      ...
      <div className="flex flex-col mb-1 bg-sky-950/15 border border-sky-500/10 rounded-xl overflow-hidden">
        <button
          type="button"
          onClick={() => setShowSectionsAccordion(v => !v)}
        ...
        {showSectionsAccordion && (
          <div className="px-3 pb-3 pt-1 space-y-2 border-t border-sky-500/10 bg-black/20">
            <div className="flex gap-1.5 pt-1 pb-1">
              <button
                type="button"
                onClick={() => updateSectionsConfig(DEFAULT_SECTIONS)}
            ...
            {[
              ...
              <label key={sec.key} className="flex items-center gap-2 cursor-pointer text-[0.76rem] text-[var(--text-main)] hover:text-sky-300 transition-colors">
                <input
                  ...
                  onChange={e => updateSectionsConfig({ ...sectionsConfig, [sec.key]: e.target.checked })}
