...
export default function IntroVideo() {
  ...
  useEffect(() => {
    ...
    const t1 = setTimeout(() => {
      setProgress(25);
      setTerminalText('SYS // SYNC ENGINE: VERIFYING CACHE...');
    }, 400);

    // Phase 3: 50% (at 800ms)
    const t2 = setTimeout(() => {
      setProgress(50);
      setTerminalText('SYS // MAP ENGINE: CALIBRATING SVGs...');
    }, 800);

    // Phase 4: 75% (at 1200ms)
    const t3 = setTimeout(() => {
      setProgress(75);
      setTerminalText('SYS // SECURITY MODULE: OK');
    }, 1200);

    // Phase 5: 100% (at 1600ms)
    const t4 = setTimeout(() => {
      setProgress(100);
      setTerminalText('SYS // DANOS APARENTES ONLINE [100%]');
    }, 1600);

    // Phase 6: Trigger Fade Out (at 2100ms)
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="fixed inset-0 z-[99999] flex flex-col items-center justify-center bg-[#020617] select-none overflow-hidden"
          style={{
            background: 'radial-gradient(circle at center, #051330 0%, #01040a 100%)',
          }}
        >
          {/* Custom CSS for holographic grid, rotate and scanner animations */}
          ...
          <div className="mt-12 w-64 flex flex-col items-center z-10">
            ...
            <div className="w-full h-[3px] bg-slate-950 border border-slate-900 rounded-full overflow-hidden relative">
              {/* Glowing bar fill (Animated via CSS Transitions for zero CPU overhead) */}
              <div
