        ...
        <div className="flex flex-wrap gap-3 mb-3">
          {orderedKeysIn(['cpf', 'cnh', 'cnhCategory']).filter(show).map(key => (
            <div key={key} className="flex-1 min-w-[min(160px,100%)]">
              ...
              {key === 'cnh' && (
                <>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6, flexWrap: 'wrap', marginBottom: 2 }}>
                    <label htmlFor="cnh-input" className={labelClasses}>Nº da Habilitação (CNH)</label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 4, cursor: 'pointer', fontSize: '0.7rem', color: 'var(--text-muted)', userSelect: 'none', whiteSpace: 'nowrap' }}>
                      <input
                        type="checkbox"
