...
export default function WizardStepVehicle({
  ...
  return (
      ...
      {(show('brand') || show('color') || show('ano')) && (
        <div className="grid grid-cols-1 sm:grid-cols-[3fr_3fr_2fr_2fr] gap-3 mb-3">
          {show('brand') && (
            <div>
          ...
          {show('brand') && (
            <div>
              <label htmlFor="model-input" className={labelClasses}>Modelo (p/ FIPE)</label>
              <input id="model-input" className={inputClasses} value={info.model || ''} onChange={e => set('model', e.target.value)} placeholder="Ex: Corolla GLi" />
            </div>
          )}
          {show('color') && (
            <div>
