...
export function createAuditEvent(
  input: Omit<VehicleEvidenceAuditEvent, 'eventId' | 'timestamp'> & { timestamp?: string },
): VehicleEvidenceAuditEvent {
  return {
    eventId: nextId('evt'),
...
export function auditEventsForReview(
  decision: ComparisonReviewDecision,
  tenantId: string,
): VehicleEvidenceAuditEvent[] {
  const markType: VehicleEvidenceAuditEventType =
