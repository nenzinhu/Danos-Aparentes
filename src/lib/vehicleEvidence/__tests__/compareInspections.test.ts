...
import { VehicleEvidenceStore } from '../memoryStore'
...
function damage(
  partial: Partial<DamageRecord> & Pick<DamageRecord, 'id' | 'partId' | 'partName' | 'type'>,
): DamageRecord {
  return {
    view: 'lateral-right',
...
describe('compareInspections — casos do MVP', () => {
  ...
  it('Adapter SavedReport → Inspection preserva danos e placa', () => {
    const report: SavedReport = {
      id: 'r1',
      savedAt: Date.parse('2026-03-15T12:00:00.000Z'),
      vehicleInfo: {
        owner: 'A',
        phone: '',
        brand: 'VW',
        plate: 'abc-1d23' as SavedReport['vehicleInfo']['plate'],
        generalNotes: '',
        interiorNotes: '',
        interiorPhotos: [],
        interiorPhotoNotes: [],
      ...
      vehicleType: 'car',
      status: 'issued',
      vehicleId: 'veh-from-report',
    }
    const insp = savedReportToInspection(report, {
      vehicleId: 'veh-from-report',
      tenantId: null,
      userId: 'user-solo',
    })
    expect(insp.tenantId).toBe('user:user-solo')
    expect(insp.plateAtInspection).toBe('ABC1D23')
    expect(insp.damages).toHaveLength(1)
    expect(insp.damages[0].photoRefs).toEqual(['blob:1'])
  })
})
