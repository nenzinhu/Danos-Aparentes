...
export type AssertCanIssueOpts = {
  /**
   * Inspetor corporativo tem permissão `issue` mas não `review`.
   * Sem bypass, o PDF fica bloqueado para sempre (UI de revisão ocultada).
   */
...
export function assertCanIssue(
  ...
  expectedContentHash?: string,
  opts?: AssertCanIssueOpts,
): void {
  if (opts?.bypassReview) return
