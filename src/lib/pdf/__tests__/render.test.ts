...
describe('Performance & Memory Cleanup (M3)', () => {
  it('schedules revokeObjectURL for blob URLs and ignores standard data/HTTP URLs', async () => {
    vi.useFakeTimers()
    const revokeMock = vi.fn()
    vi.stubGlobal('window', {})
    vi.stubGlobal('URL', { revokeObjectURL: revokeMock })

    const { revokeObjectUrlLater } = await import('../render')
    revokeObjectUrlLater('blob:http://localhost/12345', 100)
    revokeObjectUrlLater('data:image/png;base64,AAA', 100)
    revokeObjectUrlLater('https://example.com/logo.png', 100)

    expect(revokeMock).not.toHaveBeenCalled()
    vi.advanceTimersByTime(150)
    expect(revokeMock).toHaveBeenCalledTimes(1)
    expect(revokeMock).toHaveBeenCalledWith('blob:http://localhost/12345')
...
describe('resolvePdfRuntimeProfile', () => {
  it('usa escala alta no desktop com memória confortável', async () => {
    const { resolvePdfRuntimeProfile } = await import('../render')
    expect(resolvePdfRuntimeProfile({ deviceMemoryGb: 8, hardwareConcurrency: 8 })).toEqual({
      scale: 3,
      jpegQuality: 0.98,
    })
  })

  it('reduz escala em mobile e dispositivos de baixa memória', async () => {
    const { resolvePdfRuntimeProfile } = await import('../render')
    expect(resolvePdfRuntimeProfile({ isMobileUa: true })).toEqual({
      scale: 1.5,
      jpegQuality: 0.88,
    })
    expect(resolvePdfRuntimeProfile({ deviceMemoryGb: 2, hardwareConcurrency: 4 })).toEqual({
      scale: 1.5,
      jpegQuality: 0.88,
    })
    expect(resolvePdfRuntimeProfile({ deviceMemoryGb: 4, hardwareConcurrency: 4 })).toEqual({
      scale: 2,
      jpegQuality: 0.92,
    })
  })
