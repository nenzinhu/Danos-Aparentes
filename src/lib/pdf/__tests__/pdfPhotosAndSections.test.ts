...
describe('buildPhotoSection', () => {
  ...
  it('chunks photos into rows of 3, padding the last incomplete row', () => {
    ...
    // second row has 1 photo + 2 empty pad cells = 3 <td> total
    const secondRowTds = html.split('<tr style="page-break-inside:avoid')[2].match(/<td/g) ?? []
    expect(secondRowTds.length).toBe(3)
  })
