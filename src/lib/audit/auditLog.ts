...
export const AUDIT_EVENT_TYPES = [
  ...
  // Vehicle evidence platform (histórico / comparação)
  'vehicle_created',
...
/**
 * FASE 12 — eventos de auditoria ligados a um veículo (metadata.vehicle_id)
 * ou a inspeções do histórico passado.
 */
export async function listAuditEventsByVehicle(opts: {
  vehicleId: string
  inspectionIds?: string[]
  limit?: number
}): Promise<AuditLogRow[]> {
  if (!supabaseEnabled || !supabase || !opts.vehicleId) return []
  const limit = opts.limit ?? 80
  try {
    const byMeta = await supabase
      .from('audit_log')
      .select('*')
      .contains('metadata', { vehicle_id: opts.vehicleId })
      .order('timestamp', { ascending: false })
      .limit(limit)

    const rows: AuditLogRow[] = (byMeta.data as AuditLogRow[] | null) ?? []

    if (opts.inspectionIds && opts.inspectionIds.length > 0) {
      const { data: byInsp } = await supabase
        .from('audit_log')
        .select('*')
        .in('inspection_id', opts.inspectionIds.slice(0, 40))
        .order('timestamp', { ascending: false })
        .limit(limit)
      for (const row of (byInsp as AuditLogRow[] | null) ?? []) {
        if (!rows.some((r) => r.event_id === row.event_id)) rows.push(row)
      }
    }
